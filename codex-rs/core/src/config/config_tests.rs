...
bedrock_mantle_additional_regions = ["us-gov-west-1.api.aws/redirect"]
"#,
    )
    .unwrap_err();

    assert!(err.to_string().contains(
        "model_providers.amazon-bedrock: invalid Bedrock Mantle additional region `us-gov-west-1.api.aws/redirect`"
    ));
}
